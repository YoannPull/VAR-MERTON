...
if (center_data){
  ...
  data_macro_usa[, (num_vars) := lapply(.SD,
...
names(data_lags) <- paste0(rep(vars_to_lag, each = length(lag_indices)), "_lag",
                           lag_indices)
...
cat("Diagnostics de colinéarité écrits dans 'scripts/scripts_robustness/outputs_USA_Z_Factor/collinearity/'.\n\n")
