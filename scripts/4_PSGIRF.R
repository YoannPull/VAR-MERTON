# ================================================================
#   BVAR (NIW) + GIRF (Pesaran–Shin) + Injection vers Z + μ_t+h exact
...
# Variables du VAR (ordre fixé, GPR en premier pour l'identification)
...
simulate_bvar_niw <- function(Y, p = 2, nrep = 5000, seed = 123) {
  ...
  for (i in 1:nrep) {
    d <- mniw::rMNIW(1, Lambda = B_ols, Sigma = solve(XtX), Psi = S, nu = nu_post)
    draws_B[,, i]     <- d$X
    draws_Sigma[,, i] <- d$V
  }
...
# Propage un choc réduit-forme delta via les coefficients VMA (A_comp)
propagate_from_delta <- function(B, p, delta, h) {
  k <- ncol(B)
  A_comp <- build_companion(B, k, p)
  irf <- matrix(0, nrow = h + 1, ncol = k)
  irf[1, ] <- delta
...
extract_c_A_list <- extract_c_A_list
...
saveRDS(list(Psi_draws = psi_draws, B_kept = B_kept, Sigma_kept = Sigma_kept,
        ...
        file = file.path(out_dir, "var_kernel_ps.rds"))

cat("Noyau VAR (Ψ, B, Σ) sauvegardé dans:\n",
    "-", file.path(out_dir, "var_kernel_ps.rds"), "\n")
...
# ==================== GIRF (Pesaran–Shin) ==================== #
compute_girf_ps <- function(psi_draws, Sigma_kept, impulse_idx = 1, qs = c(0.05,0.16,0.50,0.84,0.95)) {
  H <- dim(psi_draws)[1] - 1
  k <- dim(psi_draws)[2]
  M <- dim(psi_draws)[4]
  variables <- dimnames(psi_draws)$variable
  girf_draws <- array(NA_real_, dim = c(H + 1, k, M),
                      dimnames = list(horizon = 0:H, variable = variables, draw = NULL))
  for (m in 1:M) {
    Psi_m   <- psi_draws[,,, m, drop=FALSE][,,,1]
    Sigma_m <- Sigma_kept[,, m]
    delta_ps <-  as.numeric(Sigma_m %*% diag(k)[, impulse_idx]) / sqrt(Sigma_m[impulse_idx, impulse_idx])
    for (hh in 0:H) girf_draws[hh+1, , m] <- Psi_m[hh+1, , ] %*% delta_ps
  }
  out <- vector("list", k)
  for (r in 1:k) {
    mat <- girf_draws[, r, , drop = FALSE]
    ...
    out[[r]] <- data.frame(horizon = 0:H,
                           ...
                           method = "Pesaran–Shin GIRF (δ_PS)")
  }
  list(draws = girf_draws, bands = do.call(rbind, out))
}

girf_res <- compute_girf_ps(psi_draws, Sigma_kept, impulse_idx = impulse_ix)
girf_draws <- girf_res$draws
girf_bands <- girf_res$bands

saveRDS(girf_draws, file = file.path(out_dir, "girf_draws_ps.rds"))
fwrite(girf_bands, file = file.path(out_dir, "girf_bands_ps.csv"))
cat("Fichiers écrits :\n",
    "-", file.path(out_dir, "girf_draws_ps.rds"), "\n",
    "-", file.path(out_dir, "girf_bands_ps.csv"), "\n")

plot_girf <- function(df, title_txt = "Pesaran–Shin GIRFs (δ_PS)") {
  ggplot(df, aes(x = horizon, y = median)) +
    geom_ribbon(aes(ymin = lower90, ymax = upper90), fill = "#7AB8F2", alpha = 0.30) +
    geom_ribbon(aes(ymin = lower68, ymax = upper68), fill = "#7AB8F2", alpha = 0.55) +
...
# ----------------------- INJECTION GIRF (PS) -> Z ----------------------- #
inject_generalized_into_Z <- function(var_kernel_rds, terms_df, impulse_idx = 1) {
  ...
  e_imp <- rep(0, k); e_imp[impulse_idx] <- 1L
  ...
  for (m in 1:M) {
    Sigma <- Sigma_kept[,,m]
    scale_factor = 10
    delta_ps <- scale_factor * as.numeric(Sigma %*% e_imp) / sqrt(Sigma[impulse_idx, impulse_idx])
    Psi_m <- Psi_draws[,,,m, drop=FALSE][,,,1]
    irfY <- matrix(0.0, nrow = H + 1, ncol = k)
    for (hh in 0:H) irfY[hh+1, ] <- Psi_m[hh+1, , ] %*% delta_ps
    acc <- numeric(H + 1)
...
inj <- inject_generalized_into_Z(
  var_kernel_rds = file.path(out_dir, "var_kernel_ps.rds"),
  terms_df = terms_df,
  impulse_idx = impulse_ix
)

saveRDS(inj$psiZ_draws, file = file.path(out_dir, "psiZ_draws_generalized.rds"))
fwrite(inj$psiZ_bands,  file = file.path(out_dir, "psiZ_bands_generalized.csv"))
fwrite(inj$terms_used,  file = file.path(out_dir, "psiZ_terms_used.csv"))

cat("Injection GIRF (PS) -> Z sauvegardée dans:\n",
    "-", file.path(out_dir, "psiZ_draws_generalized.rds"), "\n",
    "-", file.path(out_dir, "psiZ_bands_generalized.csv"), "\n",
    "-", file.path(out_dir, "psiZ_terms_used.csv"), "\n")

plot_psiZ <- function(df, title_txt = "GIRF (Pesaran–Shin) de Z") {
  ggplot(df, aes(x = horizon, y = median)) +
...
print(plot_psiZ(inj$psiZ_bands))
...
compute_mu_baseline_draws <- function(DT_var, variables, terms_df, beta0, B_kept, p, H) {
  ...
  hist_tail <- Y_all[(Tn - Lmax + 1):Tn, , drop = FALSE]
  ...
  for (m in 1:M) {
    ...
    for (r in 1:nrow(terms_df)) {
      ...
      for (hh in 0:H) {
        val <- if (hh >= lag_r) Y_fore[hh - lag_r + 1, base_j]
        else { j <- lag_r - hh; hist_tail[Lmax - (j - 1), base_j] }
...
compute_s2_one_draw <- function(Psi_draw_one, Sigma_one, terms_df, beta_vec, sigma_eta2) {
  ...
  for (hh in 0:H) {
    ...
    for (q in 0:hh) {
      ...
      if (q >= 1)  Sigma_s_cond <- Sigma_s_cond + G_hq %*% Sigma_one %*% t(G_hq)
    }
...
kernel_path <- file.path(out_dir, "var_kernel_ps.rds")
...
terms <- terms_df
beta_vec <- terms$beta
sigma_eta2 <- (summary(best_model)$sigma)^2   # <- correction explicite
...
saveRDS(list(mu_draws = mu_draws, s2_draws = s2_draws, s2_delta_draws = s2_delta_draws, terms_used = terms),
        file = file.path(out_dir, "z_mu_and_var_components_ps.rds"))
...
bp_p <- apply(U_bar, 2, function(u) lmtest::bptest(u ~ X_t[,-1])$p.value)
...
girf_pd_single <- compute_girf_pd_single(
  psiZ_draws     = readRDS(file.path(out_dir, "psiZ_draws_generalized.rds")),
  mu_draws       = mu_draws,
  s2_draws       = s2_draws,
...
# Sauvegardes
fwrite(girf_pd_single$bands, file = file.path(out_dir, "girf_pd_bands_ps_SINGLE.csv"))
saveRDS(girf_pd_single,       file = file.path(out_dir, "girf_pd_single_ps.rds"))
cat("GIRF-PD (PS) (single p,ρ) sauvegardé dans:\n",
    "-", file.path(out_dir, "girf_pd_bands_ps_SINGLE.csv"), "\n",
    "-", file.path(out_dir, "girf_pd_single_ps.rds"), "\n")

# ----------------------- Plot GIRF-PD (single) ----------------------- #
plot_girf_pd_single <- function(bands_df,
                                title_txt = "GIRF de la PD (forme fermée, Pesaran–Shin)") {
...
print(plot_girf_pd_single(girf_pd_single$bands))

# ----------------------- Sanity check signe (optionnel) ----------------------- #
psiZ_med <- inj$psiZ_bands$median
dPD_med  <- girf_pd_single$bands$median
...
# ==================== Graphique 3D des densités de PD ==================== #
library(reshape2)
library(plotly)

# On part des tirages PD delta - base
pd_draws <- girf_pd_single$draws   # (H+1) x M

# Choix d’un horizon final ou affichage complet
H <- nrow(pd_draws) - 1
M <- ncol(pd_draws)

# ---- Version surface 3D sur tous les horizons ---- #
# On crée une grille horizon × quantiles de PD
grid_size <- 50
pd_grid <- seq(min(pd_draws, na.rm=TRUE),
               max(pd_draws, na.rm=TRUE),
               length.out = grid_size)

dens_matrix <- matrix(0, nrow = H+1, ncol = grid_size)

for (hh in 0:H) {
  vals <- pd_draws[hh+1, ]
  d <- density(vals, from = min(pd_grid), to = max(pd_grid), 
               n = grid_size, na.rm=TRUE)
  dens_matrix[hh+1, ] <- approx(d$x, d$y, xout = pd_grid, rule = 2)$y
}

# Passage en data.frame long pour plotly
dens_df <- melt(dens_matrix)
colnames(dens_df) <- c("horizon", "pd_idx", "density")
dens_df$pd <- pd_grid[dens_df$pd_idx]
dens_df$horizon <- dens_df$horizon - 1  # car melt commence à 1

# ---- Graphique interactif 3D ---- #
fig <- plot_ly(dens_df,
               x = ~horizon, y = ~pd, z = ~density,
               type = "surface",
               colorscale = "Viridis")
fig <- fig %>% layout(
  scene = list(
    xaxis = list(title = "Horizon (quarters)"),
    yaxis = list(title = "Δ PD"),
    zaxis = list(title = "Densité")
  ),
  title = "Distribution simulée des ΔPD (Pesaran–Shin GIRF)"
)

htmlwidgets::saveWidget(fig, file = file.path(out_dir,"PD_density_surface.html"))
cat("Graphique interactif sauvegardé dans PD_density_surface.html\n")
